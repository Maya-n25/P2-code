...
if (digitalRead(button5) == LOW && !done5) {

  // forward motion (max safe range)
  for (int pos = 20; pos <= 150; pos += 3) {
    finalServo.write(pos);
    delay(12);
  }

  delay(200); // pause = makes brain think "completed turn"

  // slight return motion (creates full rotation illusion)
  for (int pos = 150; pos >= 40; pos -= 3) {
    finalServo.write(pos);
    delay(12);
  }
